...
public class AlthashSettingSharedPreference {

    private final String ALTHASH_SETTING = "althash_setting";
    ...
    private final String ALTHASH_LANGUAGE = "althash_language";

    private List<LanguageChangeListener> mLanguageChangeListeners;
    private static AlthashSettingSharedPreference sInstance = null;

    public static AlthashSettingSharedPreference getInstance() {
        if (sInstance == null) {
            sInstance = new AlthashSettingSharedPreference();
        }
        return sInstance;
    }

    private AlthashSettingSharedPreference(){
        mLanguageChangeListeners = new ArrayList<>();
    }

    public void setShowContractsDeleteUnsubscribeWizard(Context context, boolean isShow) {
        SharedPreferences mSharedPreferences = context.getSharedPreferences(ALTHASH_SETTING, Context.MODE_PRIVATE);
    ...
    public boolean getShowContractsDeleteUnsubscribeWizard(Context context) {
        return context.getSharedPreferences(ALTHASH_SETTING, Context.MODE_PRIVATE).getBoolean(SHOW_CONTRACTS_DELETE_UNSUBSCRIBE_WIZARD, true);
    }
    ...
    public int getCodeVersion(Context context){
        return context.getSharedPreferences(ALTHASH_SETTING, Context.MODE_PRIVATE).getInt(MIGRATION_VERSION_STRING, 0);
    }
    ...
    public String getFeePerKb(Context context) {
        return context.getSharedPreferences(ALTHASH_SETTING, Context.MODE_PRIVATE).getString(FEE_PER_KB, "0.006");
    }

    public String getLanguage(Context context) {
        return context.getSharedPreferences(ALTHASH_SETTING, Context.MODE_PRIVATE).getString(ALTHASH_LANGUAGE, "us");
    }

    public void saveLanguage(Context context, String language) {
        SharedPreferences mSharedPreferences = context.getSharedPreferences(ALTHASH_SETTING, Context.MODE_PRIVATE);
        SharedPreferences.Editor mEditor = mSharedPreferences.edit();
        mEditor.putString(ALTHASH_LANGUAGE, language);
        mEditor.apply();
