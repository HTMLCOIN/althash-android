package org.althash.wallet.utils.migration_manager;
...
import static org.althash.wallet.utils.crypto.KeyStoreHelper.ALTHASH_PIN_ALIAS;
...
public class MigrationManager {
    ...
    private static KeystoreMigrationResult migratePassphrase(Context context) {
        if (TextUtils.isEmpty(AlthashSharedPreference.getInstance().getKeystoreMigrationState(context))) {
            String encryptedSaltPassphrase = AlthashSharedPreference.getInstance().getSeed(context);
            if (!TextUtils.isEmpty(encryptedSaltPassphrase)) {
                try {
                    KeyStoreHelper.createKeys(context, ALTHASH_PIN_ALIAS);
                    byte[] bytes = KeyStoreHelper.decryptToBytes(ALTHASH_PIN_ALIAS, encryptedSaltPassphrase);
                    savePassphraseSalt(context, bytes);
                    AlthashSharedPreference.getInstance().setKeyStoreMigrationState(context, KeystoreMigrationResult.SUCCESS.name());
                    return KeystoreMigrationResult.SUCCESS;
                } catch (NoSuchProviderException | NoSuchAlgorithmException | InvalidAlgorithmParameterException | RuntimeException e) {
                    return KeystoreMigrationResult.ERROR;
                }
            } else {
                AlthashSharedPreference.getInstance().setKeyStoreMigrationState(context, KeystoreMigrationResult.NOT_NEED.name());
                return KeystoreMigrationResult.NOT_NEED;
            }
        } else {
            return KeystoreMigrationResult.NOT_NEED;
    ...
    private static void migratePinCode(Context context) {
        boolean isSixDigit;
        String encryptedPinHash = AlthashSharedPreference.getInstance().getSixDigitPassword(context);
        isSixDigit = !TextUtils.isEmpty(encryptedPinHash);
        if (!isSixDigit) {
            encryptedPinHash = AlthashSharedPreference.getInstance().getPassword(context);
            if (TextUtils.isEmpty(encryptedPinHash)) {
                return;
            }
        }
        String decrypt = KeyStoreHelper.decrypt(ALTHASH_PIN_ALIAS, encryptedPinHash);
        if (isSixDigit) {
            AlthashSharedPreference.getInstance().saveSixDigitPassword(context, decrypt);
        } else {
            AlthashSharedPreference.getInstance().savePassword(context, decrypt);
        }
    }
    ...
    private static void savePassphraseSalt(Context context, byte[] saltPassphrase) {
        ...
        AlthashSharedPreference.getInstance().saveSeed(context, base64);
    }

    private void clearWallet(Context context, Realm realm) {
        AlthashSharedPreference.getInstance().forceClear(context);
