package org.althash.wallet.ui.fragment.pin_fragment;
...
import org.althash.wallet.utils.CryptoUtilsCompat;
import org.althash.wallet.utils.PinEntryEditText;
import org.althash.wallet.utils.ThemeUtils;
import org.althash.wallet.datastorage.AlthashSharedPreference;
import org.althash.wallet.utils.CryptoUtils;
...
public class PinDialogFragment extends DialogFragment {
    ...
    @BindView(org.althash.wallet.R.id.til_wallet_pin)
    PinEntryEditText mWalletPin;

    @BindView(org.althash.wallet.R.id.tv_toolbar_title)
    FontTextView mTextViewToolBarTitle;

    @BindView(org.althash.wallet.R.id.tooltip)
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        View view = LayoutInflater.from(getActivity()).inflate(ThemeUtils.getCurrentTheme(getContext()).equals(ThemeUtils.THEME_DARK) ? org.althash.wallet.R.layout.dialog_fragment_pin : org.althash.wallet.R.layout.dialog_fragment_pin_light, null);
    ...
    private String getSixDigitPassword() {
        return AlthashSharedPreference.getInstance().getSixDigitPassword(getContext());
    }
    ...
    private String getFourDigitPassword() {
        return AlthashSharedPreference.getInstance().getPassword(getContext());
    }
            ...
            ((MainActivity) getActivity()).checkSensorState(new MainActivity.SensorStateListener() {
                ...
                public void onRequest(MainActivity.SensorState sensorState) {
                    if (sensorState == MainActivity.SensorState.READY) {
                        mTextViewToolBarTitle.setText(org.althash.wallet.R.string.confirm_fingerprint_or_pin);
                        prepareSensor();
                    } else {
                        mTextViewToolBarTitle.setText(org.althash.wallet.R.string.enter_pin);
                    }
                }
            });
        } else {
    ...
    private void confirmError(String errorText) {
        if (getDialog() != null) {
            ...
            tooltip.setTextColor(ContextCompat.getColor(getContext(), org.althash.wallet.R.color.accent_red_color));
    ...
    private class FingerprintHelper extends FingerprintManagerCompat.AuthenticationCallback {
        ...
        public void onAuthenticationSucceeded(FingerprintManagerCompat.AuthenticationResult result) {
            ...
            String encoded = AlthashSharedPreference.getInstance().getTouchIdPassword(getContext());
    ...
    private void confirm(String pin) {
        Long banTime = AlthashSharedPreference.getInstance().getBanTime(getContext());
        ...
        if (isCorrect) {
            ...
            confirmError(getContext().getString(org.althash.wallet.R.string.incorrect_pin));
        }
    }
    ...
    private void changeBanState(boolean isCorrect) {
        ...
        if (isCorrect) {
            ...
            failedAttemptsCount = AlthashSharedPreference.getInstance().getFailedAttemptsCount(getContext());
            failedAttemptsCount++;
            if (failedAttemptsCount == 3) {
                Long currentTime = Calendar.getInstance().getTimeInMillis();
                Long banTime = currentTime + mBanTime;
                AlthashSharedPreference.getInstance().setBanTime(getContext(), banTime);
                failedAttemptsCount = 0;
                mPinCallBack.onError(getString(R.string.sorry_please_try_again_in) + " " + mBanTime.intValue() / 60000 + " " + getString(R.string.minutes));
            }
        }
        AlthashSharedPreference.getInstance().setFailedAttemptsCount(getContext(), failedAttemptsCount);
    }
