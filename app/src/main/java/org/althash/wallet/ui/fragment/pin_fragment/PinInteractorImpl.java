...
import static org.althash.wallet.utils.crypto.KeyStoreHelper.trimEndSpaces;
...
class PinInteractorImpl implements PinInteractor {
    ...
    private String getFourDigitPassword() {
        String encryptedPinHash = AlthashSharedPreference.getInstance().getPassword(mContext);
        return KeyStoreHelper.decrypt(ALTHASH_PIN_ALIAS, encryptedPinHash);
    }

    @Override
    public String getSixDigitPassword() {
        return AlthashSharedPreference.getInstance().getSixDigitPassword(mContext);
    }

    @Override
    public Integer getFailedAttemptsCount() {
        return AlthashSharedPreference.getInstance().getFailedAttemptsCount(mContext);
    }

    @Override
    public Long getBanTime() {
        return AlthashSharedPreference.getInstance().getBanTime(mContext);
    }

    @Override
    public void setFailedAttemptsCount(int failedAttemptsCount) {
        AlthashSharedPreference.getInstance().setFailedAttemptsCount(mContext, failedAttemptsCount);
    }

    @Override
    public void setBanTime(long banTime) {
        AlthashSharedPreference.getInstance().setBanTime(mContext, banTime);
    }

    @Override
    public void saveSixDigitPassword(String password) {
        AlthashSharedPreference.getInstance().saveSixDigitPassword(mContext, password);
    }
    ...
    public void savePassphraseSaltWithPin(String pin, String passphrase) {
        ...
        AlthashSharedPreference.getInstance().saveSeed(mContext, encryptedSaltPassphrase);
    }

    @Override
    public byte[] getSaltPassphrase() {
        String encryptedSaltPassphrase = AlthashSharedPreference.getInstance().getSeed(mContext);
        return Base64.decode(encryptedSaltPassphrase, Base64.DEFAULT);
    }

    @Override
    public String getTouchIdPassword() {
        return AlthashSharedPreference.getInstance().getTouchIdPassword(mContext);
    }

    @Override
    public void saveTouchIdPassword(String password) {
        AlthashSharedPreference.getInstance().saveTouchIdPassword(mContext, password);
    }
    ...
    public void setKeyGeneratedInstance(boolean isKeyGenerated) {
        AlthashSharedPreference.getInstance().setKeyGeneratedInstance(mContext, isKeyGenerated);
    }
