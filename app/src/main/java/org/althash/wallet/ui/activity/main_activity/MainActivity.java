...
import org.althash.wallet.ui.activity.splash_activity.SplashActivity;
import org.althash.wallet.ui.base.base_activity.BaseActivity;
...
import org.althash.wallet.ui.fragment.wallet_fragment.WalletFragment;
import org.althash.wallet.ui.fragment.wallet_main_fragment.WalletMainFragment;
import org.althash.wallet.utils.CustomContextWrapper;
import org.althash.wallet.utils.FontManager;

import org.althash.wallet.ui.fragment.profile_fragment.ProfileFragment;
import org.althash.wallet.ui.fragment.send_fragment.SendFragment;
...
import static org.althash.wallet.ui.base.base_fragment.BaseFragment.BACK_STACK_ROOT_TAG;
...
public class MainActivity extends BaseActivity implements MainActivityView {
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (intent != null && intent.getAction() != null) {
            switch (intent.getAction()) {
                case AlthashIntent.OPEN_FROM_NOTIFICATION:
                    openRootWallet();
                    break;
                default:
                    break;
    ...
    public boolean checkTouchIdEnable() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (AlthashSharedPreference.getInstance().isTouchIdEnable(getContext())) {
    ...
    public void initializeViews() {
        ...
        switch (intent.getAction()) {
            case AlthashIntent.SEND_FROM_SDK:
                getPresenter().setSendFromIntent(true);
                mAddressForSendAction = intent.getStringExtra(AlthashIntent.SEND_ADDRESS);
                mAmountForSendAction = intent.getStringExtra(AlthashIntent.SEND_AMOUNT);
                mTokenAddressForSendAction = intent.getStringExtra(AlthashIntent.SEND_TOKEN);
                break;
    ...
    protected void attachBaseContext(Context newBase) {
        super.attachBaseContext(CustomContextWrapper.wrap(newBase, AlthashSettingSharedPreference.getInstance().getLanguage(newBase)));
    }

    public AlthashApplication getAlthashApplication() {
        return (AlthashApplication) getApplication();
    }
    ...
    public Realm getRealm(){
        return ((AlthashApplication)getApplication()).getRealm();
    }
}