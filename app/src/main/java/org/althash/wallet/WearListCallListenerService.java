...
import static org.althash.wallet.AlthashApplication.REALM_NAME;
...
public class WearListCallListenerService extends WearableListenerService implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, MessageApi.MessageListener {
    ...
    public void onMessageReceived(MessageEvent messageEvent) {
        ...
        if(messageEvent.getPath().contains("/get_history")) {
            ...
            currentAddress = AlthashSharedPreference.getInstance().getCurrentAddress(getApplicationContext());
            sendData();
        } else {
            stopSelf();
        }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    final String balance = AlthashSharedPreference.getInstance().getBalanceString(WearListCallListenerService.this);
                    final String uncBalance = AlthashSharedPreference.getInstance().getUnconfirmedBalanceString(WearListCallListenerService.this);
                    final String address = AlthashSharedPreference.getInstance().getCurrentAddress(WearListCallListenerService.this);
                    Gson gson = new Gson();
